...
2.  **加载插件**：
    ...
    *   选择本目录下的 `advanced_extension` 文件夹。
    *   加载成功后，您应该能在扩展栏看到插件图标。

## 使用指南

1.  **初始化设置**：
    *   初次点击插件图标，会提示您打开管理面板。
    *   在管理面板中，点击 "Setup / Link Folder"，选择一个本地文件夹（建议新建一个空文件夹，例如 `my_property_data`）用于存放数据。
    *   授权浏览器对该文件夹的读写权限。
...
3.  **统一查看房源信息**
    ...
    *   打开Dashboard后，会自动分析提取所有房源信息，包括

4.  **日常管理**：
    *   在表格中查看对比房源。
    *   **隐藏/显示列**：觉得列太多太乱？点击顶部的蓝色标签把不需要的列（如 Size 或 Images）点灰即可隐藏。
    *   **输入通勤时间**：在 CommuteTime 列直接输入时间（如 "30"）。
    *   **导出**：需要做进一步分析或分享给其他人时，点击右上角的 "Export CSV" 按钮。
