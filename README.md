...
*   **自动提取房源信息**：自动抓取房源页面中的 Price, Size, Bed/Bath, Address, Title, Type 等核心数据。
*   **交互式数据面板**：
...
3.  **初始化本地保存地址&授权读写**
    *   安装完插件后，会弹出"Welcome to Property Manager"初始介绍页
    ![Setup](readme_imgs/2_setup.jpeg)
    *   在介绍页中，点击 "Select Folder"，选择一个本地文件夹（建议新建一个空文件夹，例如 `my_property_data`）用于存放数据。
    *   授权浏览器对该文件夹的读写权限。
    ![Linked](readme_imgs/3_linked.jpeg)
    *   安装完成后，您应该能在扩展栏看到插件图标，pin一下更方便使用。
